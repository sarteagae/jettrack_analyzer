void plotQAandDijetHistograms() {
  ...
  //  const char* file3 = "histos_Data_wZDC_pPb_gamma_proton_wtFRGcuts_OCt28_pPb_Data_8160GeV_Quench_ak4PFJetAnalyzer_nojettrig_Jptmin_10p0_Jptmax_8160p0_Jetamin_N1p0_Jetamax_1p0_TOTAL.root";
   ...
   const char* file1 ="histos_gam_protn_FRGles2p5_BRGles2p5_FRGaftjets_jetsAftBRG_March24_total.root"; //gam_protn MC
   //const char* file2 = "histos_gam_pom_FRGles2p5_BRGles2p5_FRGaftjets_jetsAftBRG_March24_total.root";    //gam_pom MC
   const char* file2 = "histos_DATA_PAEGJets_HF7_FRGles2p5_BRGles2_FRGaftjets_jetsAftBRG_Apr2.root";    //gam_pom MC
   
    const char* file3 = "histos_DATA_PAEGJets_FRGles2p5_BRGles2_FRGaftjets_jetsAftBRG_Apr2_all.root";
    const char* file4 ="histos_QCD_FRGles2p5_BRGles2p5_FRGaftjets_jetsAftBRG_March24_total.root";
    
    const char* qa_folder = "QA_histograms";
    const char* qa_hist_name = "hist_BRG_aft_FRGcuts";
    ...
    //qa_hist2->Scale(0.014796*0.1); //0.013796 fact from old normalization//0.1972 // factor from xsec from pythia gam-proton
    qa_hist2->Scale(1.0); 
    qa_hist3->Scale(1.0); 
    ...
    qa_hist2->SetMarkerStyle(kFullCircle);
    qa_hist2->SetMarkerColor(kBlue);
    qa_hist2->SetLineColor(kBlue);
    qa_hist2->SetMarkerSize(0.7);
   ...
   // qa_hist1->GetYaxis()->SetRangeUser(0, 100000000); // Set ratio range
    qa_hist3->GetYaxis()->SetTitle("counts");
    qa_hist3->GetYaxis()->SetTitleSize(0.03); 
    //qa_hist1->GetYaxis()->SetTitleOffset(0.35);  // Adjust the Y-axis title offset to align it
    qa_hist3->GetXaxis()->SetTitleOffset(1.2);
    qa_hist3->GetXaxis()->SetTitle("Gap_{proton} ");
    qa_hist3->GetXaxis()->SetTitleSize(0.03);
    
   // qa_hist1->SetMaximum(qa_hist3->GetMaximum());
   // qa_hist3->SetMaximum(1e4);
    //qa_hist3->SetMinimum(1e1);
   ...
   // legend->AddEntry(qa_hist1, "PYTHIA8 photo-ND", "l");
    ...
    legend->AddEntry(qa_hist3, "Data HF<4GeV", "p");
    legend->AddEntry(qa_hist2, "Data HF<7GeV", "p");
    legend->SetFillStyle(0);  // Set fill style to transparent
    legend->SetBorderSize(0); // Remove the border around the legend
    legend->SetTextSize(0.02); // Reduce text size
    legend->Draw();
...
pPbText.DrawLatex(0.75, 0.92, "pPb 8.16TeV (2016)");  // Position pPb label
    ...
    c1->SaveAs("QA_Histogram_BRG_HF7_Apr2.pdf");
    delete c1;
